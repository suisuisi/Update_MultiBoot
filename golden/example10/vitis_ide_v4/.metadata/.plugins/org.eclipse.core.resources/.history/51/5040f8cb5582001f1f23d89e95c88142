...
void printf_usb(char *s)
{
	while (*s)
	{
		XUartLite_SendByte(XPAR_UARTNS550_0_BASEADDR,*s);
		while (XUartNs550_IsTransmitEmpty(XPAR_UARTNS550_0_BASEADDR) == true) {
		};
		s++;
	}
	return;
...
int read_usb_timeout(int timeout)
{
	...
	while (XUartNs550_IsTransmitEmpty(XPAR_UARTNS550_0_BASEADDR) && timeout)
	{
		timeout--;
	}

    if (timeout) rc = rc_byte_nb(XPAR_UARTNS550_0_BASEADDR);
